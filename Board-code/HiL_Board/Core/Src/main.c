...
#include <stdio.h>		// Used for memset-function for UART echo
#include <string.h>		// Used for memset-function for UART echo
...
/* Definitions for Task_UART_Echo */
osThreadId_t Task_UART_EchoHandle;
const osThreadAttr_t Task_UART_Echo_attributes = {
  .name = "Task_UART_Echo",
  .stack_size = 256 * 4,
...
uint8_t huart3_Rx_data[4];


//Callback functions for DMA UART
...
//void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
//{
//	HAL_UART_Receive_IT(&huart3, huart3_Rx_data,10);// Restart receiving in interrupt mode.
...
/* USER CODE BEGIN Header_StartTask_UART_Echo */
/**
  * @brief  Function implementing the Task_UART_Echo thread.
...
void StartTask_UART_Echo(void *argument)
{
  ...
  for(;;)
  {
	...
	//__HAL_UART_SEND_REQ(&huart3, UART_RXDATA_FLUSH_REQUEST);	// Flush buffer, MIGHT BE UNNCESSARY! LEAVE UNCOMMENTED OUT UNLESS PROBLEMS OCCUR.
